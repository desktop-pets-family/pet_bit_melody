...
class Notes8bit(NoteEquivalence8bit):
    """_summary_
    ...
    def __init__(self, note: Union[List[int], int] = 80, duration: float = 3.0, amplitude: float = 0.5, sample_rate: int = 44100, base_track: Union[AudioSegment, None] = None) -> AudioSegment:
        ...
        self.eight_bit_frequencies = Notes8bit.NOTE_EQUIVALENCE
    ...
    def test_range(file_name: str = "./8bit_range.wav", silence_duration: float = 0.2) -> None:
        ...
        eight_bit_range = Notes8bit.generate_8bit_frequencies()

        # Generate the audio
        audio = None
        for frequency in eight_bit_range:
            print(f"Generating note for frequency {frequency}")
            audio = Notes8bit.add_note(
                audio, frequency, Notes8bit.NOTE_DURATION)
            audio = Notes8bit.add_note(
                audio,
                Notes8bit.MUTE,
                silence_duration
        ...
        # Export the audio
        Notes8bit.save(audio, file_name)
    ...
    def test_notes(file_name: str = "./8bit_notes.wav", silence_duration: float = 0.2) -> None:
        ...
        notes = Notes8bit.get_available_notes()

        audio = None
        for note in notes:
            print(f"Generating note for {note}")
            audio = Notes8bit.add_note(audio, note, Notes8bit.NOTE_DURATION)
            audio = Notes8bit.add_note(
                audio,
                Notes8bit.MUTE,
                silence_duration
